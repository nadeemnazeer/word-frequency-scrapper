...
class FreqScrapper:
    ...
    def _compute_freq_dict(self,url,ngram,top,max_level):
        """ 
        ...
        #Generate ngrams/freq for particular url/page
        fdist = self._compose_ngrams(raw, ngram)
        
        #Accumulate and aggregate the frequencies as we crawl URLS
        ...
        #Look for internal links, anc check for max_level limit
        ...
        for link in links:
            if link not in self.url_visited:
                self._compute_freq_dict(link,ngram,top,max_level)
    ...
    def get_freq_words(self,ngram,top,max_level,sort="desc"):
        ...
        if sort == "asc":
            reverse=False
        self._compute_freq_dict(self.url, ngram,top,max_level)
        
        #Sort the dict by default desc and return top N needed
